...
const generateEyeStates = (
  ...
  while (cursor < totalFrames) {
    ...
    const recoveryPadding = Math.round(random() * fps * 0.1);
...
const createSyntheticVolumeArray = (
  ...
  script.items.forEach((item, index) => {
    ...
    const phases: Array<{duration: number; text: string}> = [
      ...
      {duration: segment.answerDelayFrames, text: ""},
      {duration: segment.answerFrames, text: resolvedAnswerText},
    ];
...
export const CharacterOverlay: FC<Props> = ({
  character,
  theme,
  storyboard,
  script,
